...
echo "Selected process = $proc_choices"
#
...
echo "number of proc = $num_proc"
echo "Assimilated domain=" $domain
#
...
#echo $run_wrfda
#echo "bufr $run_wrfda_buf"
#echo "obs $run_wrfda_obs"
#
...
#----------------------------------------------
 if ($run_real) ; then
   echo "main_shell start: real"
   #bash ./run3_real.sh $wuudir
   echo "main_shell end: real"
 else
...
#----------------------------------------------
 if ($run_wrfda) ; then
   if ($run_wrfda_buf) ; then
     echo "main_shell start: wrfda 3dvar bufr"
     #bash ./run4_buf_da.sh $start_time $wuudir $domain
     echo "main_shell end: wrfda 3dvar bufr"
   fi
   if ($run_wrfda_obs) ; then
     echo "main_shell start: wrfda 3dvar obs"
     #bash ./run4_obs_da.sh $start_time $wuudir $domain
     echo "main_shell end: wrfda 3dvar obs"
   fi
 else
   echo "skip to run real"
 fi
